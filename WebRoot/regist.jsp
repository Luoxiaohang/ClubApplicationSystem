...
<body onload=loading()>
	<script>
		...
		function loading(){
			$.ajax({
	      		type : "GET",  
	          	url : "/ClubApplicationSystem/School/getSchoolList",
	          	success : function(response) {
	          		var result = eval(response).list;
	          		if(result){
	          			var optionSize = result.length;
	          			var selectOptions="";
	          			for(var i=0;i<optionSize;i++){
	          				selectOptions += '<option value ="'+ result[i].id + 
			           			'">' + result[i].name +
			           			'</option>';
	          			}
	          			$("#schoolId").append(selectOptions);
	          		}
	          	}
	      	});					
		}
		...
		function checkInfo(){
		   	...
		   	$.ajax({
           		type : "POST",  
               	url : "/ClubApplicationSystem/User/regist",
		...
		function checkEmail(value){
			$.ajax({
           		type : "POST",  
               	url : "/ClubApplicationSystem/User/checkEmail",
               	data:'Email='+value,
               	success : function(msg) {
               		var obj = eval(msg);
                   	if (obj.success) { 
			...
			if(value.trim() == ""){
				console.log("账号为空");
		     	$("#account").val("");
		   		...
		   		$.ajax({
	           		type : "POST",  
	               	url : "/ClubApplicationSystem/User/checkAccount",
	               	data:'account='+value,
	               	success : function(msg) {
	               		var obj = eval(msg);
	                   	if (obj.success) { 
		...
		function checkPhone(value){
			$.ajax({
           		type : "POST",  
               	url : "/ClubApplicationSystem/User/checkPhone",
               	data:'phone='+value,
               	success : function(msg) {
               		var obj = eval(msg);
                   	if (obj.success) { 
